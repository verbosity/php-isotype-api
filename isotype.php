...
function quickvis_build_image($variables, $adjustments) {
 ...
 // SPECIAL CASE - COUPLE
  if ($variables['type'] == 'couple') {
  	$variables['numicons']  = 1;
  	$variables['grad_dir']  = 'vertical';
	$variables['man']    = quickvis_paths('man');
	$variables['woman']  = quickvis_paths('woman');	
	$variables['visstyle'] = 'total_gradient';
    $variables['gradients'] = create_gradients($variables,$adjustments);
			// set scale from format
   if ( isset($_GET['format']) && ( $_GET['format'] == 'thumbnail') ) {
	  $variables['scale']  = '0.3';
   }
     else { $variables['scale']  = '0.5';
   }
	// icons are in a row, so calculate width as a combination of number of icons
	$variables['width']  = ( $variables['man']['width'] + $variables['woman']['width']);
	
	//height is the height of the largest icon ( plus buffer )
	$variables['height'] = $variables['man']['height'] +50 ;
	
	//apply adjustments for header/ label/key
	$variables['height'] = ($variables['height'] * $variables['labels_adjust']  );
	$variables['height'] = ($variables['height'] * $variables['header_adjust']  );
	$variables['height'] = ($variables['height'] * $variables['key_adjust']     );
	$variables['height'] = ($variables['height'] * ($variables['scale']        * 2)          );
	$variables['width']  = ($variables['width']  * $variables['key_adjust']    * ($variables['scale']*2) );
	
	// colour/ image stuffies
	// in essence the gradient point ( if used ) needs to be caluclated ( both in which icon it happens in, and where in the icon )
	if($variables['visstyle']  =='icon_gradient') {
	$adjustments['gradient']=TRUE;	
	
	}
		if($variables['visstyle']  =='total_gradient') {
	$adjustments['gradient']=TRUE;	
	...
	$quickvis_svg_builder .= quickvis_svg_header($variables);
	//print_r($variables);

	// scaling/format wrapper
	$quickvis_svg_builder.=' <g  transform=" scale(0.5)"> ';
	

	
	// build values
	$quickvis_step=2 ;
	$quickvis_step_size=$variables['man']['width'];
	$quickvis_start_offset=10;
	$quickvis_counter=1;
	//build icons
	while ($quickvis_counter <= $quickvis_step){
	// need to do some colour stuffies here
	$adjustments['color']='blue';
	$adjustments['yadj']=50;
	$adjustments['xadj']=(($quickvis_counter-1) * $quickvis_step_size)-100;
	if ($quickvis_counter == 1 ) {
		    $adjustments['type']='man';	    
    }
	if ($quickvis_counter == 2 ) {
		$adjustments['type']='woman';
   }
	...
	// end scaling wrapper
	$quickvis_svg_builder.=' </g> ';
	
	$quickvis_svg_builder .= quickvis_svg_footer() ;
  }
 // End Special case -couple
 

//SPECIAL CASE - FAMILY  
//-------------------------------------------------------------------
  if ($variables['type'] == 'family'){
    $variables['numicons']  = 1;
     $variables['grad_dir'] = 'horizontal'	;
	$variables['man']    = quickvis_paths('man');
	$variables['woman']  = quickvis_paths('woman');
	$variables['visstyle'] = 'total_gradient';
	 $variables['gradients'] = create_gradients($variables,$adjustments);
   ...
   if ( isset($_GET['format']) && ( $_GET['format'] == 'thumbnail') ) {
	...
	// DEV NOTE - above is old code might need to be deleted
	//build image
	$quickvis_svg_builder  = '';
	$quickvis_svg_builder .= quickvis_svg_header($variables);
	...
	$adjustments['color']='blue';
	$adjustments['yadj']=50;
	$adjustments['xadj']=(($quickvis_counter-1) * $quickvis_step_size)-220;
	if (($quickvis_counter == 4) or($quickvis_counter == 3) ) {
	   $adjustments['yadj']=345;	
	   $adjustments['xadj']=(($quickvis_counter-1) * ($quickvis_step_size)-20);
	}	
   ...
   if ($quickvis_counter == 3 ) {
		    $adjustments['type']='man';	
		    $quickvis_svg_builder.=' <g  transform=" scale(0.7)"> ';    
    }
	if ($quickvis_counter == 4 ) {
		$adjustments['type']='woman';
		$quickvis_svg_builder.=' <g  transform=" scale(0.7)"> ';
   }
